...
export const WeatherProvider = ({ children }: Props) => {
  ...
  const handleChangeSelectedCity = async (value: string) => {
    ...
    try {
      if (value !== null) {
        ...
        handleGetTodayWeather(newLocation);
        handleGetHourlyWeather(newLocation);
        handleGetWeeklyWeather(newLocation);
      ...
      console.error("handleChangeSelectedCity", err);
    }
  };
  ...
  const handleLocalStorageCheck = () => {
    try {
      ...
      console.error("handleLocalStorageCheck", err);
    }
  };
