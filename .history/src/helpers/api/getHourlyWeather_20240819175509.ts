...
export const getHourlyWeather = async (location: Location) => {
  try {
    const URL = `https://api.open-meteo.com/v1/forecast?latitude=${location.latitude}&longitude=${location.longitude}&hourly=temperature_2m,weather_code&daily=sunrise,sunset&forecast_days=3`
    ...
    const tempArray = response.temperature_2m.slice(nowHours, 25 + nowHours)
 

    return timeArray.map((value : string, index: number) => {  
      return {
            temp: Math.round(tempArray[index]),
            time: value.slice(11),
