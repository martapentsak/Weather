...
export const getTodayWeather = async (location: Location) => {
  ...
  try {
    ...
    console.log(new Date(response.daily.sunrise[0]).getHours())  
    return {
      ...
      sunriseHour:  console.log(new Date(response.daily.sunrise[0]).getHours()) ,
      sunsetHour:  response.daily.sunrise[0].slice(10),
    };
  } catch (error) {
